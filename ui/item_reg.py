...
class ItemRegPage:
    def __init__(self, master, item, on_update=None):
        ...
        self.d_day = item.get('Dday') or item.get('d_day', 'D-0')
        self.expiry_date = item.get('exp_date') or item.get('expiry_date', '2026-01-01')
        self.unit = item.get('unit', '개')
        self.added_date = item.get('reg_date') or item.get('added_date', '')
    ...
    def info_left_frame(self):
        ...
        initial_q = self.item_data.get('quentity') or self.item_data.get('quantity', 1)
        ...
        self.category_entry = tk.Entry(self.category_frame, textvariable=tk.StringVar(value=self.item_data.get('category', '')), bg=self.bg_color, fg='black', font=("Arial", 12))
    ...
    def info_right_frame(self):
        ...
        def control_event(item_date_str):
            ...
            def on_confirm():
                ...
                d_day_diff = (expiry_dt - today).days
